...
$tmpValue = @'
[{ "OU": "OU=Users,OU=HelloID Training,DC=veeken,DC=local"}]
'@ 
...
<# Begin: DataSource "AD-user-generate-table-report-mailbox-hidden" #>
$tmpPsScript = @'
try {
...
Invoke-HelloIDDelegatedForm -DelegatedFormName "AD - Report - Accounts where the mailbox is hidden" -DynamicFormGuid $dynamicFormGuid -AccessGroups $delegatedFormAccessGroupGuids -Categories $delegatedFormCategoryGuids -UseFaIcon "True" -FaIcon "fa fa-info-circle" -returnObject ([Ref]$delegatedFormRef) 
<# End: Delegated Form #>
...
'@ 
	...
	Invoke-HelloIDAutomationTask -TaskName "AD-export-report-accounts-mailbox-hidden" -UseTemplate "False" -AutomationContainer "8" -Variables $tmpVariables -PowershellScript $tmpScript -ObjectGuid $delegatedFormRef.guid -ForceCreateTask $true -returnObject ([Ref]$delegatedFormTaskGuid) 
} else {
	Write-ColorOutput Yellow "Delegated form 'AD - Report - Accounts where the mailbox is hidden' already exists. Nothing to do with the Delegated Form task..." 
}
